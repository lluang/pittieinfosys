...
library(HH, quietly = TRUE) # new diagnostic functions
...
-  Purpose of a framework - Provide direction to a project when you do not necessarily know the answer.
-  Structure a report so that information is where the reader expects it to be.
...
CoNVO
=============
-  Context. Who is the client customer and their goals. 
-  Need.  Should be a question (data oriented) that you can answer.
    -  What are some other options for need? What kinds of need can be answered through your other courses?
-  Vision. How you (the engineer/analyst) will meet the client's need.
    -  At the beginning of the project, the Vision gives you a roadmap and a process for doing the work.
    -  Provides some confirmation that when you are done, you will have a useful answer.
    -  If you do not a have an affirmative answer, at least you have a negative answer that will suggest the next step.
-  Outcome.  What are the possible client responses.
    -  Process improvement
    -  Statistical or predictive model analysis
...
- Maximum of 8 pages.
- CoNVO
  -  Vision should be the a first draft of your realized data presentation
  -  Outcome should be a hypothetical outcome based on what you actually saw in your visualization.
- Data discussion: Volume, Velocity, Variety, Veracity
...
Linear regression is a form of model
====================================
...
- `glm` also exists and is often used classifiers or for additional evaluation options.
...
Confidence and prediction intervals for iris using `ci.plot`
========
...
QQ-plot
=======
-  Quantile-Quantile plot (QQ-plot) compares two distributions against each other.
-  For model diagnostics compare the sorted residuals against what they would be if they were normally distributed.
-  In general, are two distributions the same. (i.e. for probability input modeling, does the data match a candidate distribution)
...
-  We can get confidence and prediction intervals for predictions/forecasts on new points using `predict` 
```{r}
