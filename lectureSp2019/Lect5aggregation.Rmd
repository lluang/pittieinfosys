---
title: 'Lecture 5: Data Aggregation: Mutate and summarise'
author: "IE 0015 Spring 2019"
date: "February 2019"
output:
...
- We may want to aggregate (summarise)
...
-  We often want to look at summarised data across groups, not individual data points.
...
-  Usually, we can do things using the `mutate`, `group_by`, `summarise` functions from `dplyr`. This is for when those do not work (e.g. there is a function we need to use that does not work with `summarise`)
...
- Use `group_by` and `summarise` to create data summaries
...
-  While `group_by()` and `summarise()` take groups of observations, `mutate()` takes one observation at a time.
...
-  summarise data frame split by factors
...
```{r}
acsh %>% group_by(tenurevalues) %>%
  summarise(householdincome = mean(householdincome, na.rm=TRUE),
...
-  summarise a columns based on defined groups
