...
-  *Lists can be heterogenous, they can contain multiple data types.
...
-  Identify either intergers or doubles by using `is.numeric()`
...
-  All floating point numbers (doubles) are appproximations. Integers can be exact.
...
-  Each element of a chacter vector is a string
...
- Two programming pardigms apply differnt forms of iteration
...
- Make explicit statements on what to do
- Loop constructs include *for loops* and *while loops*
...
-  Replaces the old object with the new result
...
- use concatonate `c()` to add to the vector with each iteration
```{r}
means <- c(0, 1, 2)
...
- Better to creat a list, then combine into a single vector when done
...
str(unlist(out))
```
- The vector is createed using `unlist(out)` which takes a list of vectors and flattens them into a single vector.
...
Removing duplicates
===================
- If functions are very much identical, you could add an arguement that captures the difference
```
...
- If a function requires additional parameters, you add them after the function name.
  - Arguements are data, function name, other parameters
