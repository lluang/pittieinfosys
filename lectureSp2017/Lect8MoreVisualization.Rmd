---
title: 'Lecture 8: More Visualization'
...
```
```{r}
p = ggplot(dsmall,aes(carat, price, color = color, shape=cut)) + geom_point()
show(p)
```
...
- Start from the simplist possible plot, then add details.

```{r}
t = ggplot(dsmall,aes(carat, price, color = color)) 
t1 = t + geom_point()
t2 = t1 + theme_bw()
```
***
...
- Use multiple geom statements in the same plot using different data.

```{r}
u = ggplot(dsmall)
u1 = u + geom_point(aes(table/50, price), color="red")
u2 = u1 + geom_point(aes(carat, price), color="blue") + xlab("Table/50 or Carat")
show(u1)
```

Multiple dimensions on a single plot
===================

- Add aesthetics that represent different variables in the data.
- Make relationships or correlation visually apparent.
- Start with carat (weight) vs. price, add color and cut.
...
Add diamond color and cut to the plot
===
```{r}
p = ggplot(dsmall,aes(carat, price, color = color, shape=cut)) + geom_point()
show(p)
...
Summarizing on top of data
====

-  Create a dataframe with an associated summary (we will cover dplyr next week)
-  Use multiple geom statements (specify that you are using a new data frame)

```{r}
library(dplyr)
dsmallsummarize<- group_by(dsmall, cut) %>%
  summarize(meanprice = mean(price, na.rm=TRUE), sdprice = sd(price, na.rm=TRUE))
```

Plot by using multiple geom statements
==================

-  geom statement can specify another dataset

```{r}
v = ggplot(dsmallsummarize, aes(x=cut))
v = v+ geom_errorbar(aes(ymin=meanprice-sdprice, 
                    ymax=meanprice+sdprice), width=0.25)  +
  ggtitle("Price of diamonds grouped by cut")
show(v)
```

Add the points in addition to summary
==========

- Visualize what color diamonds cause the outliers.
- Consider if color should be considered along with cut.

```{r}
v1 = v+  geom_point(data=dsmall, aes(x=cut, y=price, color = clarity), size = 1, alpha=.2) +
