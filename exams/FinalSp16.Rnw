...
\item kNN (how will you define *nearest*?)
...
\begin{enumerate}
\item Mean value imputation would probably be condition on the street name. However this will always give a location in the middle of the length of the street, and bike lanes may not extend the entire street, so it would not locate the crash correctly.
...
\question
Pittsburgh has 6 police zones (Figure \ref{fig:policezones}), where each zone is comprised of a number of neighborhoods.  In addition to the crash data, you have census data with the population for every neighborhood (i.e. there are two fields, census$neighborhood and census$population) and a function that identifies which neighborhood a crash occurs in based on either latitude and longitude or street address (so you can easily connect it with the crash data). A Pittsburgh City Councilman has requested that a graphic of crashes, crashes with injuries, and crashes with fatalities per capita (number divided by population) for the past five years by police zone be provided to show the distribution of crashes across the city. Assume that population is relatively static over the five years.

\begin{parts}
\part[12]
What type of graph should be used to show this information and why? (Your answer should discuss and compare at a minimum; pie charts, some version of bar chart, some version of line chart.  Remember to include your choice.)
...
\caption{Pittsburgh Police Zones. \label{fig:policezones}}
\end{figure}

\part[18]
Sketch a visualization and write out the flow chart/process map that would create that visualization starting from reading data tables.
