...
\centering
{\bf IE 0015 Spring 2017 Midterm}
...
\item
  The exam is four pages (including cover). In addition there are three pages of reference containing the data dictionaries and an example of the data from both the Pittsburgh Police Blotter and the Pittsburgh Police Incident database, and a map of Pittsburgh with neighborhoods and police zones for your reference.
\item
...
The Alliance of Police Accountability (APA) is a grassroots organization dedicated to criminal justice reform, specializing in improving community and police relations through advocacy, education, and policy.

One event where the APA becomes involved is when police action leads to a civilian fatality in a case where there is a perception that there was no threat to life due to the suspect.

On January 31, 2016, Mr. Bruce T. Kelley Jr. and another man were involved in a fight with Port Authority officers who were confronting them while drinking in a Port Authority Subway shelter in Wilkinsburg, PA . Two officers were injured, and in the ensuing pursuit and confrontation, Mr. Kelley was shot and killed after killing a Port Authority K-9. The APA is interested in arguing that the subjects actions did not in themselves normally result in the police use of lethal force.  The APA has available all police incident and arrest reports after identifying information has been removed.  It would also have access to incident reports that involve subject resistance and police use of force.


\begin{parts}
\part   Create a CoNVO statement from the point of view of an analyst preparing a report as a consultant for the APA (i.e. you represent the APA).
...
The  (CPRB) is an independent agency of the City of Pittsburgh set up to investigate citizen complaints about improper police conduct. It is made up of representatives appointed by either the City Council or the Mayor. 
...
Specifically, that the ratio of arrests to incidents for different categories of crimes is different across neighborhoods.

Using the Police Arrest data, answer the following.

\begin{parts}

\part[5]  Currently, the "Offenses" column in the arrest data is not tidy. How is it not tidy?

\part[10]  Create a process map that will modify the arrest data to make it tidy. Note that as part of this you should separate the code given in the "Offenses" column from the description.
...
2. The following:
  \begin{itemize}
  \item Create a new table with the incident number and the offenses column (could include Hierarchy as well).
  \item  Split the offenses column by the '|'.
  \item  Create a new row with each new offense and the original incident number (CCR).
  \item  Split the Offense number from the text using the first space.
  \item  You now have a table with one row for each offense that has the CCR, offense number and the office description.
  \end{itemize} 
\end{solution}
...
\part[20]  The "Hierarchy" column in the Blotter table groups contains the highest (most serious) offense of an incident/arrest. Assume that you have a table that maps the offense code to the Hierarchy code.  Develop a process map that will calculate the number of incidents and arrests by offense in each police zone as well as the ratio of arrests to incidents.

\part[15]  Create a process map that will allow you to compare the ratio of arrests to incidents by Police Zone for the ten most common incidents (defined in Hierarchy) in that zone.  You may start with your answer to part a.  

\part[15]  Sketch a visualization that is based on the process map in part b. This visualization should indicate both the ratio of arrests to incidents as well as the total number of incidents.  Explain how your visualization fulfills all of these requirements.


\begin{solution}[4in]

1.  Process map for part a and b follows:

  \begin{itemize}
  \item Mutate: add a column to the arrest data table by joining the arrest table to the hierarchy table using the offense number.
  \item For both arrest and blotter table, group by hierarchy and police zone.
  \item  For both arrest and blotter table, summarize by counting the number of arrests/incidents for each combination of hierarchy and police zone. This gives the raw number.
  \item  For each combination of hierarchy and police zone, divide the number of arrests by the number of incidents. This gives the ratio of arrests to incidents for each police zone.
  \end{itemize}

3.  Starting with the number of arrests and incidents and their ratio.

  \begin{itemize}
  \item For each police zone, order the hierarchies by the number of incidents.
  \item For each police zone, filter by the top 5 hierarchies.
  \item  For a visualization, I would use a map of Pittsburgh that had the police zones identified. In each, I would put in a bar chart that depicted the total number of incidents behind the total number of arrests, so the two bars would total to the number of incidents, and the bars could be defined as arrests vs incidents in excess of arrests. This would show both proportion and the amount.
  \end{itemize}
\end{solution}
