...
<<>>=
pittsburghassess <- PropertySalesTransactions %>%
  filter(SALEDESC=="VALID SALE") %>%
  left_join(PropertyAssessmentsParcelData, by="PARID") %>%
  ...
  mutate(bedbath = paste(BEDROOMS, FULLBATHS, HALFBATHS),
         saleyear=year(SALEDATE.x),
         overassessment = PRICE-COUNTYTOTAL) 
pittsburghassess %>% head()
@
\end{solution}
...
\begin{parts}
\part  The county assessor wants a visual representation that allows comparison of all sales prices compared to the total assessment value for homes with both an assessed value and a sales price of less that \$1 million.  You may have to manipulate the data to do this and you may use your answer to question 2 as a starting point. To make this comparison, design a visualization that covers the following variables and explain how it enables the comparison:
\begin{enumerate}
    \item COUNTYTOTAL 
    \item PRICE 
    \item Year of sale
...
ggplot(pittsburghassess) +
  geom_point(aes(PRICE, COUNTYTOTAL, alpha = 0.1)) + 
  facet_wrap(~saleyear)
@
\end{solution}
