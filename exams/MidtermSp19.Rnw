...
The University of Pittsburgh has recently opened a Community Engagement Center (CEC) in the Homewood neighborhood of Pittsburgh.  Councilman Rev. Ricky V. Burgess represents Council District 9 which includes Homewood (he is councilman based on municipal elections). He wishes to discuss with the Pittsburgh Police zone commanders their level of enforcement on firearms laws because he believes the high quantity of firearms is a contributor to violent crime in his district and is asking the University of Pittsburgh (you) for assistance.

(a)  Create a CoNVO statement as an analyst assisting Councilman Burgess in tracking police firearms seizures in the council district he represents.
...
(a)  Create a CoNVO statement as an an analyst assisting the Women's Center tracking police firearms seizures in Pittsburgh.

Pittsburgh Police Domestic Violence Unit
...
(a)  Create a CoNVO statement as an an analyst assisting the Pittsburgh Police Domestic Violence unit tracking police firearms seizures in Pittsburgh.
...
Councilman Burgess wishes to have a chart that shows the monthly firearms seizures in each neighborhood by year and compare this to the incidence of violent crimes in each neighborhood by month and year since 2015.  Assume that you already have the violent crimes data in whatever form you need.  

\begin{parts}
  \part Draw a flow chart of a process that prepares the firearms seizure data so that each line shows the total number of firearms seized in each neighborhood in Council District 9 in a month, sorted by neighborhood and date.
  \part Sketch out a plot that does this.  
  \part Explain how your plot fits the criteria provided (i.e. state the visual cue for each part of the specification)
\end{parts}

\begin{solution}
Flow chart should include a filter (council district), a group by (neighborhood, month, year), and summation (count).

Example plot.  One plot (facet) per year, stacked bar plot with firearms seizures x axis has months, each color (stacked) is a neighborhood. Add a line plot to show violent crimes (potentially with another axis)

Points off for missing an aspect (by neighborhood, by month, or violent crimes)
...
pittsburghfirearmsseizures %>% 
  ...
  summarize(weaponsseizures = sum(total_count)) %>%
  arrange(neighborhood, year, month) %>% head()
ggplot(seizuresperneighborhood) + geom_bar(aes(x=month, fill=neighborhood)) + facet_grid(year ~ .) + scale_x_discrete()
@
\end{solution}

Domestic violence victim advocates working with police are interested in comparing firearms seizures with the rate of court orders to turn in firearms for each zone. Assume that you have the data from court orders to turn in firearms by police zone in whatever form that you need.

\begin{parts}
  \part Draw a flow chart of a process that prepares the firearms seizure data so that each line shows one month's firearms seizures in each police zone in Pittsburgh since January 2015, sorted by polize zone and date.
  \part Sketch out a plot that does this.  
  \part Explain how your plot fits the criteria provided (i.e. state the visual cue for each part of the specification)
\end{parts}

\begin{solution}
Flow chart should include a filter (council district), a group by (police zone, month, year), and summation (count).

Example plot.  One plot (facet) per year, stacked bar plot with firearms seizures x axis has months, each color (stacked) is a zone Add a line plot to show PFA orders (potentially with another axis)

Points off for missing an aspect (by police zone, by month, or violent crimes)
...
Table \ref{tbl:cashflow} shows the expected cost outlay and returns over time for a project.  The discount (interest) rate is 20\%.
...
Year & 0 & 1 & 2 & 3 & 4\\  
\hline
Initial Investment  & -200 & 0 & 0 & 0 & 0 \\
Sales & 0 & 100 & 110 & 120 & 130\\
Costs & 0 & -30 & -35 & -40 & -45\\
\hline
\end{tabular}
\caption{Cash flow \label{tbl:cashflow}}
\end{table}


A regional supermarket chain is deciding whether to install a tewgit machine in each of its stores.  Each machine costs \$250,000. Projected cash flows per machine is in Table \ref{tbl:tewgit}.
...
\hline
Cost  & -250 &  &  &  &  &  \\
Sales &  & 250 & 300 &  300 & 250 & 250 \\
Operating Expenses &  & -200 &  -200 &  -200 &  -200 &  -200 \\
\end{tabular}
\caption{Tewgit machine cash flow 1000s of dollars \label{tbl:tewgit}}
\end{table}
...
\part Draw the flow chart that demonstrates how to find the present value of the entire cash flow presented in the table.
\end{parts}
