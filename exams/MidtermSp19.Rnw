...
\begin{parts}
  \part Draw a flow chart of a process that prepares the firearms seizure data so that each line shows one month's firearms seizures in each neighborhood in Council District 9.
...
\begin{parts}
  \part Draw a flow chart of a process that prepares the firearms seizure data so that each line shows one month's firearms seizures in each police zone in Pittsburgh since January 2015.
  \part Sketch out a plot that does this.  
  \part Explain how your plot fits the criteria provided (i.e. state the visual cue for each part of the specification)
\end{parts}
...
You have the following table that shows the expected cost outlay and returns over time for a project.  The discount (interest) rate is 20\%.
...
\end{tabular}
\caption{Tewgit machine cash flow 1000s of dollars}
\end{table}
