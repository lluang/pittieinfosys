...
3.  Process map follows:
  ...
  \begin{itemize}
  \item Mutate: add a column to the arrest data table by joining the arrest table to the heirarchy table using the offense number.
  \item For both arrest and blotter table, group by heirarchy and police zone.
  \item  For both arrest and blotter table, summarize by counting the number of arrests/incidents for each combination of heirarchy and police zone. This gives the raw number.
  \item  For each combination of heirarchy and police zone, divide the number of arrests by the number of incidents. This gives the ratio of arrests to incidents for each police zone.
  \end{itemize}

4.  Starting with the number of arrests and incidents and their ratio.

  \begin{itemize}
  \item For each police zone, order the heirarchies by the number of incidents.
  \item For each police zone, filter by the top 5 heirarchies.
...
<<echo=False>>=
Blotter <- read_csv("~/courses/datascience/infosys2015/data/PittsburghPoliceBlotter.csv")
Arrest <- read_csv("~/courses/datascience/infosys2015/data/PittsburghPoliceArrest.csv")

@

\centering
{Table 1. Incidents from Blotter}
<<echo=FALSE, results = tex>>=
Pittsburghblotter <- Blotter[1:7,]
Pittsburharrest <- Arrest[1:7,]
blottertab <- xtable(Pittsburghblotter, caption="Archived Incidents", label="tbl:blotter")
arresttab <- xtable(Pittsburghblotter, caption="Arrests", label="tbl:arrests")
print(blottertab, floating=FALSE, include.rownames = FALSE)
@
...
<<echo=FALSE, results = tex>>=
print(arresttab, floating=FALSE, include.rownames = FALSE)
@
