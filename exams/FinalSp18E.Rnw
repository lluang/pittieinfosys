...
\begin{itemize}
  \item Context - insurance company needs to make money on insurance policies
  \item Need - Show relationship between PLI violations and fire risk.
  \item Vision - Show relationship between number of violations and likelihood of fires on that block.  Plots showing frequency. 
...
\part[10]  Because Councilman Burgess is only interested in property neglect fires, he is not interested in cooking or interior fires.  Draw the processmap that identifies all fires that involve a building, structure,or trash in Council District 9. Note: look for the words "building", "structure", or "trash" in the Type description. Check your answer against the list above to consider if you will include Type descriptions that you cannot see.

\part[10]  Notice that the addresses in the fire and incidents databases are recorded differently (see example in attachment. Note that the examples should match each other when you are done).  Draw a process map that will convert the addresses so that they do match each other.  Note that addresses ending in "00" indicate that it is not an exact address, but identifies the block on the street.  (i.e. when you are done, the field(s) with the address will match using `join` commands that identify equalities)
...
\begin{parts}
\part[10]  Comment on the quality of the model given the four diagnostic plots and the regression summary table. Note p-value, spread of residuals over x-axis, qq-line, scale-leverage (presence or lack of cook's distance). Note R-squared not applicable to evaluating a single model.
...
\begin{solution}[2in]
\begin{itemize}
  \item  Answer to evaluation should note low p-value. r-squared not applicable as only one model is being evaluated.  Should also note following qq-plot, patterns in residuals (or lack), and if there are outliers, evidenced by the cook's distance appearing in scale-location plots
  \item Adding a variable: should choose variables by finding a pattern in residuals when sorted by that variable.  Adding that variable to the model will then allow the model to include variation due to that variable.  Specifically, do NOT add variables where the residuals are random, as that variable imparts no information.
\end{itemize}
\end{solution}
\newpage
...
of college graduates who support off-shore drilling in California is different than that of non-college graduates (control group). using simulation to the 5\% significance level.
...
#iint(tailnulltrue)
hist(oilout, breaks=30, main = "Simulation of 438 college students from (a)\n asked if they support off shore drilling in California", xlab = "Watchers")
@
...
Process map:
\begin{enumerate}
  \item  Calculate the probability of a college student supporting off-shore drilling.
