...
Clean Water Action is interested in the toxic releases in Allegheny County by the chemicals industry (identified through the Industry\_Sector field = `Chemicals`), their forms (release, energy recovery, treatment, recycling, identified in 8.x), and locations within Allegheny County.

\begin{parts}
\part Write an appropriate CoNVO statement for Clean Water Action.
\end{parts}

\newpage
\question [25]  Clean Water Action wants to identify the 10 largest pollution locations in the Chemical industry.  However, these facilities are actually complexes, so there are multiple addresses on the same street with the same corporate owner (in Facility Name.  Here is an example of the items in Facility\_Name and Street\_Address.
...
@
\begin{parts}
\part Assume that you have a function, \texttt{findmunicipality(lat, long)}, that can identify the municipality and council district that a given latitude and longitude falls into.  Write a process map that can calculate the total releases by the Chemical industry by facility and list the top 10 in rank order (largest first).  A facility is identified as all locations with the same owner (FACILITY\_NAME), Street (note: you cannot use STREET\_ADDRESS as is), and council district.
\end{parts}
\begin{solution}
\begin{itemize}
