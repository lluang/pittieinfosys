...
The mission of the Western Pennsylvania Conservancy (WPC) is to provide Western PA with clean waters and healthy forests, wildlife, and natural areas.  They are most known for creating green spaces and gardens in small, unused spaces in urban areas. 

The WPC is concerned about the level of biodiversity in the City of Pittsburgh. In particular, it is concerned that trees in specific neighborhoods may be dominated by a single *genus* or *family* (group of species) of tree. It wants to identify neighborhoods that have low diversity so it can help take action to increase the biodiversity of the neighborhood.
...
Here, the context is the WPC wants to maintain nature in the city.
...
The WPC wants to know where tree biodiversity is low so it can take action to increase biodiversity.
...
An example would be plotting the distribution of tree families by neighborhood (such as a stacked bar plot). Neighborhood bar plots dominated by a single family would be candidates for further attention. Another way is to give the percent of trees of the dominant type. A higher percentage of the dominant type would be a candidate for action.
...
Here, the WPA would identify neighborhoods with high concentrations where it could plant trees of different types or remove some members of the dominant genus.
...
\begin{parts}
\part Write/draw a process map that can generate this dollar value by neighborhood.  Note that the first part of the process map should combine the fields that quantify the dollar benefit of air quality from a tree into a single column.  Note that the first part of the process map should combine the fields that quantify the dollar benefit of from a tree into a single column. Note: you only need to name three value fields on the process map, I'll assume you can identify the others.
...
\pagebreak
\question[20]
Below is a bar graph with the distribution of the six most common tree genus (families) in each of the six public works divisions.
...
\begin{parts}
  \part Design and sketch a single visualization that shows:
    \begin{itemize}
      ...
      \item  Distribution of trees by genus within the public works division.
