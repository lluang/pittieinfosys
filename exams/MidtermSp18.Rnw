...
The Pittsburgh Citizen Police Review Board (CPRB) is an independent agency of the City of Pittsburgh set up to investigate citizen complaints about improper police conduct. It is made up of representatives appointed by either the City Council or the Mayor. 

One of the responsibilities of the CPRB is to represent the interests of the general public in investigations of police misconduct and to provide transparency into police activity.  The CPRB has available all police incident and arrest reports, which includes all incidents that involve subject resistance and police use of force.
...
\begin{parts}
\part   Create a CoNVO statement from the point of view of an analyst preparing a report as a consultant for the CPRB (i.e. you represent the CPRB).
...
The Pittsburgh Citizens Police Review Board (CPRB) wants to investigate claims that the Pittsburgh Police Department provides better services to some of Pittsburgh's 99 neighborhoods compared to others. 
Specifically, that the ratio of arrests to incidents for different categories of crimes is different across neighborhoods.

Using the Police Blotter, answer the following.

\begin{parts}

\part[5]  Currently, the "Offenses" column in the police blotter is not tidy. How is it not tidy?

\part[10]  Create a process map that will modify the police blotter to make it tidy. Note that as part of this you should separate the code given in the "Offenses" column from the description.
...
<<echo=FALSE>>=
#PittsburghPoliceArrest <- read_csv("../data/PittsburghPoliceArrest.csv")
#PittsburghPoliceBlotter <- read_csv("../data/PittsburghPoliceBlotter.csv")
@
