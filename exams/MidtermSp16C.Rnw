...
\begin{parts}
\part What additional information do you need that is not currently in the records provided by the Gun Violence Archive? (see page 4 for example records from the Gun Violence Archive)
\part Write out a procedure (data manipulation flow) that will enable comparisons between the incidence of accidental shooting incidents and the incidents of mass shootings using the three tables from the Gun Violence Archive that are on page 5. Note that many incidents can exist in more than one table.

\begin{solution}[4in]

1.  The data provided does not include the ages of the victims.
2.  Methodology should include the following:
    1.  Combine accidental deaths and injuries and remove duplicates and count.
    3.  Count mass shootings.
    4.  Compare the two.
\end{solution}
...
The Cornell University Lab of Ornithology has sponsored an activity called Project Feederwatch since 1987. Volunteers document observations of bird species at defined locations chosen by the volunteer from November through April (often backyards or a local park).
One use of this data is the impact of climate change on migratory bird behavior. Specifically, one species, Ruby-throated Hummingbirds, that typically migrates from the United States to Central America and Mexico is now being observed wintering in the southern United States.

Project Feederwatch collects the following data for each observation period:

\begin{enumerate}
  \item  Date
  \item  Time (morning, afternoon, evening)
  \item  Location (latitude and longitude)
  \item  Type of location. Urban/suburban/rural, residence, park
  \item  Duration of observation period (minutes spent observing birds)
  \item  Species observed.  Bird species and number observed.  Maybe none.
  \item  Weather conditions (temperature, rain/snow)
\end{enumerate}


\begin{parts}
\part Sketch as data presentation (tables or charts/graphs. Maybe more than one) that can relate the observations of Ruby-throated Hummingbirds over 10 years.  Include a title and legend/axis labels as appropriate. Note that the charts and maps do not have to be accurate.
...
\begin{solution}[3in]
The solution will likely involve a combination of maps and graphs. It should cover locations identified, density, and show that there is change over years. 
\end{solution}
...
\begin{landscape}
This exam will make use of the data from the gun violence archive on mass shootings, acidental injuries, and accidental deaths.  A sample from each of these tables is below. Note that if an incident involved an accidental death and accidental injury, that incident would appear in both Table 1 and Table 2.
...
\centering
{Table 3. Mass Shootings. Four or more deaths and injuries in a single incident}
