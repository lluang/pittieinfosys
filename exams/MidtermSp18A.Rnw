...
The mission of the Western Pennsylvania Conservancy (WPC) is to provide Western PA with clean waters and healthy forests, wildlife, and natural areas.  They are most known for creating green spaces and gardens in small, unused spaces in urban areas.  Because they want to focus efforts on where they can do the most good, they are interested in knowing what parts of the city are lacking in trees so they can create additional mini-gardens in those neighborhoods.


\begin{parts}
\part   Create a CoNVO statement from the point of view of an analyst assisting the WPC in identifying some of Pittsburgh's 90 neighborhoods to focus future efforts. Note: each section should have 2-4 sentences.
...
\part  Write a process map that makes a list of the top 10 tree family by total count for the city of Pittsburgh. Remember to remove "Vacant".
\end{parts}
...
\begin{parts}
\part Write/draw a process map that can generate this dollar value by neighborhood.  Note that the first part of the process map should combine the fields that quantify the dollar benefit of air quality from a tree into a single column. Note: you only need to name three value fields on the process map, I'll assume you can identify the others.
\part Sketch a visualization that shows the dollar benefit of air quality by neighborhood.
\end{parts}
...
\begin{parts}
  \part Design and sketch a single visualization that shows:
      \begin{itemize}
      \item  Total trees per public works division.
      \item  Distribution of trees by family within the public works division.
      \item  Relative location of public works division.
    \end{itemize}
