...
ggplot(toxicalleghenyyearly, aes(x=YEAR)) + geom_line(aes(y=Releases), color="black") +
   ...
   scale_colour_manual(name="Total Releases",values=y, 
                  guide = guide_legend(override.aes=aes(fill=NA))) + xlab("Year (Releases-black, Energy recovery-blue, \n Recycle-green, Treatment-purple, Onetime release-red")
...
2.  The client in problem 1 wants to identify the 10 largest pollution locations.  However, these facilities are actually complexes, so there are multiple addresses on the same street with the same corporate owner (in Facility Name.  Here is an example of the items in Facility_Name and Street_Address.

```{r, echo=FALSE, warning=FALSE, message=FALSE}
toxicallegheny %>% select(FACILITY_NAME) %>% head()
toxicallegheny %>% select(STREET_ADDRESS) %>% head()
```

a. Assume that you have a function that can identify the municipality and council district that a given latitude and longitude falls into.  Write a process map that can calculate the total releases by facility and list the top 10 in rank order (largest first).  A facility is identified as all locations with the same owner (FACILITY_NAME), Street (note: you cannot use STREET_ADDRESS as is), and council district.
...
2.  The Clean Air Council wants to identify the 10 largest Clean Air Act Chemical pollution facilities.  However, these facilities are actually complexes, so there are multiple addresses on the same street with the same corporate owner (in Facility Name.  Here is an example of the items in Facility_Name and Street_Address.
...
a. Assume that you have a function `findmunicipality(lat, long)` that can identify the municipality (city or town) that a given latitude and longitude falls into.  Write a process map that can calculate the total releases of Clean Air Act Chemicals by facility and list the top 10 in rank order (largest first).  A facility is identified as all locations with the same owner (FACILITY_NAME), Street (note: you cannot use STREET_ADDRESS as is), and municipality.
...
2.  The PennEnvironment wants to identify the 10 largest pollution facilities.  However, these facilities are actually complexes, so there are multiple addresses on the same street with the same corporate owner (in Facility Name.  Here is an example of the items in Facility_Name and Street_Address.
...
a. a. Assume that you have a function `findmunicipality(lat, long)` that can identify the municipality (city or town) that a given latitude and longitude falls into.  Write a process map that can calculate the total releases by facility and list the top 10 in rank order (largest first).  A facility is identified as all locations with the same owner (FACILITY_NAME), Street (note: you cannot use STREET_ADDRESS as is), and council district.
