...
-  Vision - The vision needs to be a presentation of data that can answer the Need and lead to a decision.  One common mistake is to overly summarize the data.  Summarizing the data hides information, so we only do it after we have determined that a variable is not important.  A sketch (an informal drawing or a mockup) of a chart can help here. Pie charts are especially bad at showing information, bar charts are slightly better.  Pie charts are often used to give justification to a decision already made, but when exploring data it is important to be able to see the variation in it. Because it allows for decisions and courses of action that allows for recognizing that "one size does not fit all."  The big mistake here is to be vague (e.g. 'a data analysis' or 'a chart).  This needs to be concrete so that the client knows what she will be getting from you. Another major mistake is to not address the problem presented (which was generally two options on where to put effort)
...
2.  Methodology should include the following:
    ...
    4.  Compare the two. This usually involved a summary (e.g. count of victims, count of incidents) of two groups of data. The comparison needed to be fair (i.e. there needed to be grouping of filtering to ensure fairness, usually by date)
\end{solution}
...
One problem is in the use of percentages.  Because percentages are calculations off a base, they cannot be added when the percentages were fractions of different starting point. In addition, percentage graphs are always based on 100\%.
\end{solution}
