...
Common mistakes include not checking for duplicates (explicitly stated in the question), not combining killed and injured in accordance with the definition of mass shooting, not having two summaries that can be compared, and not making sure that the summaries were comparable (e.g. checking that the date ranges were the same using group by or filter)
\end{solution}
