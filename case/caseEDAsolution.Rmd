...
##datadirectory <- "E:\\Documents\\courses\\datascience\\pittieinfosys2015\\case\\"
datadirectory <- "."

RegionEx <- read_excel(file.path(datadirectory, "StudentData_RegionEx_(2010-9-22).xlsx"),
...
```{r}
RegionEx <- RegionEx %>%
  mutate(scheduledflighttime = Scheduled.arrival.time - Scheduled.departure.time)
ggpairs(RegionEx %>% select(Airline,  Scheduled.departure.time, scheduledflighttime, Arrival.delay.in.minutes, Delay.indicator, Day.of.Week, Route.Code, Number.of.passengers))
```
...
Look at routes and airlines
================

```{r}
ggplot(RegionEx, aes(x=Arrival.delay.in.minutes, fill=Airline)) + geom_histogram( position="dodge2" ) + facet_grid(Airline~.)
```


```{r}
RegionEx %>% group_by(Airline, Route.Code) %>%
  summarize(flights = n())
```
...
RegionEx %>% group_by(Airline, Route.Code) %>%
  summarize(flights = n(),
            meanflight = mean(scheduledflighttime),
            medianflight=median(scheduledflighttime),
            meandelay = mean(Arrival.delay.in.minutes),
            mediandelay = median(Arrival.delay.in.minutes)
            )
```
```{r}
ggplot(RegionEx, aes(x=Arrival.delay.in.minutes, fill=Airline)) + geom_histogram( position="dodge2" ) + facet_grid(Route.Code + Airline~.)
```

```{r}
ggplot(RegionEx, aes(x=scheduledflighttime, fill=Airline)) + geom_histogram( position="dodge2" ) + facet_grid(Route.Code + Airline~.)
```

```{r}
ggplot(RegionEx, aes(x=scheduledflighttime, , y=Arrival.delay.in.minutes, color=Airline, shape=Airline)) + geom_point() + facet_grid(Route.Code~.)
```