...
In the `tidyr` package there are two Tuberculosis data sets `who` and `population` (you used them in Lab 3).  Look at the `tidyr` help files (R Studio -> Packages tab -> tidyr) to see what each contains. To load and view the last few rows of the datasets, you should enter:
...
```{r}
who_new <- who[,c("country", "iso2", "year", "new_sp_m014",  "new_sp_m1524","new_sp_m2534", "new_sp_m3544", "new_sp_m4554", "new_sp_m5564", "new_sp_m65", "new_sp_f014", "new_sp_f1524", "new_sp_f2534", "new_sp_f3544", "new_sp_f4554", "new_sp_f5564", "new_sp_f65",  
...
1.  [1] Create a subset of tidywho that only includes the ASEAN nations.  Name this `whoasean`. Note that the list of nations and the corresponding ISO2 codes is provided below. You will need the package `countrycode` to make this work.
...
The following code will take `whoasean` (your answer from question 1) and generate columns for the `diagnosismode`, `agerange`, and gender (i.e. what you did in homework 1)
...
Group and plot the total rate of diagnosis of TB per year for each country in a form that allows for comparisons across countries. 

  a. Draw a process map that illustrates how to do this.
  b. Show at least two visualizations. Note that it should include a time series for each country in a form that distinguishable using black and white printing.
...
3.  [12] You are interested in how the different countries have been changing the ratio of use of the various diagnosis modes have been changing per year.

  a.  Draw a process map to create the dataset with the ASEAN countries and diagnosis mode by year.
  b.  Design and implement three visualizations to compare the use of different diagnosis modes by each ASEAN country over time. Note: the country can be referenced using ISO2 codes. The diagnosis should be indicated using the full words (not a code). The three visualizations should use different *geom* and should use a printer friendly color map. The data dictionary is at http://www.who.int/tb/country/data/download/en/
