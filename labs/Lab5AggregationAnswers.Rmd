...
Alternative using `dplyr` verbs
```{r}
by_education <- group_by(completebf, education, marital)
summarise(by_education, avgweeks = mean(BFEEDWKS))
```
```{r}
group_by(completebf, education, marital) %>%
  summarise(avgweeks=mean(BFEEDWKS))
```
Remark:   
