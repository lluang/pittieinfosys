---
title: 'Lab 4: Simple Graphs with ggplot2'
...
col_types = cols(CASEID = col_character(), 
                 HOWENDDK = col_factor(),
                 WHENDIED_M2 = col_number(),
                 WHENDIED_Y2 = col_number(),
                 WHENDIED_M3 = col_number(),
                 WHENDIED_Y3 = col_number(), 
                 cmkidied2 = col_number(),
                 cmkidied3 = col_number(),
                 CSECPLAN = col_number(),
                 CSECMED4 = col_number(),
                 CSECMED5 = col_number(),
                 WEEKSDK = col_number(),
                 BIRTHWGT_LB3 = col_number(),
                 BIRTHWGT_OZ3 = col_number(),
                 FRSTEATD_N3=col_number(),
                 FRSTEATD_P3 = col_number(),
                 AGEQTNUR_N3 = col_number(),
                 NOWPRGDK = col_number(),
                 AGEQTNUR_P3  = col_number(),
                 DK2GEST = col_number(),     
                 WHYNOUSE4 = col_number(),  
                 WHATMETH04 = col_number(),
                 BABYSEX3 = col_number(), 
                 ANYNURSE3 = col_factor(), 
                 AGEQTNUR3 = col_number(), 
                 LEGAGREE2  = col_number(),
                 FEDSOLID2 = col_number(), 
                 WHYNOUSE5 = col_number(),
                 PREGEND3 = col_factor(),
                 LIVEHERE3 = col_factor(),
                 ALIVENOW3 = col_factor()
                 ))
...
1. Include both R code and output, or TA will not be able to give you any points if there is difference between your graph and the preferred answer. (i.e. code allows for partial credit)
2. "==" is used for comparisons while "=" indicates an assignment (this problem is why we use '<-' for assignments). 
...
```{r}
ggplot(FemPreg[FemPreg$OUTCOME==1,]) + geom_histogram(aes(PRGLNGTH))+xlab("Pregnancy length (weeks)") + ggtitle("Pregnancy length - Live births")
```
```{r}
ggplot(FemPreg[FemPreg$OUTCOME==6,]) + geom_histogram(aes(PRGLNGTH))+xlab("Pregnancy length (weeks)") +ggtitle("Current pregnancy")
```
```{r}
ggplot(FemPreg[FemPreg$OUTCOME==4,]) + geom_histogram(aes(PRGLNGTH)) +xlab("Pregnancy length (weeks)") + ggtitle("Miscarriages")
```
...
2. Whenever you are not 100% sure about the usage of a function, use command like "?ggplot" or "?as.factor" to get a help page.(add a ? before the command you want to use) It will help you a lot when you have questions about the order or format of function parameters.
...
```
```{r}
ggplot(birthweightdata) + geom_jitter(aes(factor(pregordr), BIRTHWGT_LB1))  + xlab("Pregnancy order") + ylab("Birthweight (lbs)") + ggtitle("Birthweight by birth order - jitter plot")
```

```{r}
ggplot(birthweightdata) + geom_jitter(aes(factor(pregordr), BIRTHWGT_LB1), alpha=(1/20))  + xlab("Pregnancy order") + ylab("Birthweight (lbs)") + ggtitle("Birthweight by birth order - jitter plot with alpha")
```
