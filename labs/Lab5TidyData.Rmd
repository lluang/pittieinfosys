...
To save effort, only use the variables corresponding to new positive pulmonary smears. (*sp* code is in the name) (The code that does this is given for you. While it runs as is, you will need to modify this.)
...
Note: For turning in the flow chart, after writing out the chart on a separate piece of paper, the students can either:

1.	Take a picture then insert it into the knitted Word Document.
2.	Turn in multiple files to Courseweb--one for the knitted .rmd file and one for the image file.

```{r}
who_new_sp <- who %>% 
  select("country", "iso3", "year", 
         "new_sp_m014", "new_sp_m1524",
         "new_sp_m2534", "new_sp_m3544", 
         "new_sp_m4554", "new_sp_m5564", 
         "new_sp_m65", "new_sp_f014", 
...
c.  Compare the table in 2(c) and the table in 3(b).  Which way of presenting the data do you prefer and why?
