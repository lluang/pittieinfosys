...
job <- data.frame(job = c(1, 2,3, 4, 5, 6, 7, 8), 
                  processingtime = c(2, 3, 2, 1, 4, 3, 2, 2),
                  duedate = c(5, 4, 13, 6, 12, 10, 15, 19))
```
...
b. [2] Write methods that implement the following scheduling strategies and calculates the total lateness (time past due that an order is completed).  Apply this to the same example created in (a):
...
c. [2] Simulate each method 20 times.  Return a mean and 95% confidence interval of the mean lateness for each of the three scheduling strategies.  Which is the best?
...
```{r}
fifolateness <- fifolate(jobsim)
eddlateness <- eddlate(jobsim)
sptlateness <- sptlate(jobsim)
```
...
```
```{r 2c results}
latesimresults <- data.frame((fiforesults = fifolatesim, 
                              eddresults = eddlatesim,
                              sptresults =  sptlatesim))
ggplot(latesimresults) + geom_boxplot(aes(fiforesults)) + geom_boxplot(aes(eddresults)) + geom_boxplot(aes(sptresults))
```