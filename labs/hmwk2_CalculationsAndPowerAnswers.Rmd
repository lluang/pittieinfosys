---
title: 'Homework 2: Calculations and Functions'
author: "IE 0015"
...
This assignment involves working with data, writing functions, and the act of data measurement. The data measurement tasks are based on topics that you will later cover in your human factors course.  Note: Over the break, you should start by looking at Problem 2, as it lists body measurements that you need to take.

This assignment builds on the discussion articles "What is Fitness?" (first article from beginning of semester) and "What can exercise science learn from CrossFit?" (assigned for week we return from break)  The purpose of those articles was to state in general terms a method of applying scientific principles to evaluating exercise and fitness.  In this homework assignment, you will operationalize the concepts in those articles. 

Note: we will discuss in class, there is more than one way to interpret this for some of the workouts (true in real life engineering), but your answers should have the correct units (from physics, dimensional analysis) and be internally consistent.
...
1. [0] Load the affiliate list and the CrossFit Open files as needed based on the following rule:

-  Use the first letter of your last name. 
-  If you are in a team of two, use the last name of the person whose first name comes first alphabetically to determine men or women, use the first name of the person whose last name comes first for 2018 or 2019.

Last (family) name:  A-L: Men,  M-Z: Women
First (given) name:  A-L: 2018, M-Z: 2019
...
Men_Rx_2018 <- read_csv(file.path(datadirectory, "Men_Rx_2018.csv"),
col_types = cols(Affiliate_id = col_character(),
User_id = col_character()))
Women_Rx_2018 <- read_csv(file.path(datadirectory, "Women_Rx_2018.csv"),
col_types = cols(Affiliate_id = col_character(),
User_id = col_character()))
...
Women_Rx_2019 <- athlete2019list %>%
  filter(division=="Women") %>%
  left_join(scores2019, by="competitorid")
Men_Rx_2019 <- athlete2019list %>%
  filter(division=="Men") %>%
  left_join(scores2019, by="competitorid")
# Note data set is large, you may want to do your work on a sample of the data.  
# Comment out when you are done so you submit your final answer based on the full dataset

Women_Rx_2018 <- Women_Rx_2017 %>% sample_frac(0.1)
Women_Rx_2019 <- Women_Rx_2018 %>% sample_frac(0.1)
Men_Rx_2018 <- Men_Rx_2018 %>% sample_frac(0.1)
Men_Rx_2019 <- Men_Rx_2019 %>% sample_frac(0.1)
```
...
2. [1] Look up (search using the internet) the workouts that comprise the 2018 and 2019 CrossFit Opens (best place is games.crossfit.com).  In this assignment, you will need to calculate power output in foot-pounds.  This requires being able to measure distance traveled.  You are only interested in vertical distance traveled in your calculations of power.  Measure the distances while standing.  Assign the results to variables that you will be using to calculate work. 
...
3. [2] Average power over time.
    ...
    b.  Write a function for each workout that calculates average power over time given weight, distance and time. You may look up *work* and *power* in Wikipedia to see the mathematical definitions for both. (you will use this later, so design the function and its parameters accordingly) (Note: writing functions is covered in the readings from https://swcarpentry.github.io/r-novice-inflammation/)

For cardio components, you may ignore scores based on counts or distance. For scores based on energy consumption (unit = cals) (row, bike, or ski erg), note that 1 cal = 3085.96 foot pounds or 4186.8 joule (or Newton meters).
For $N$ rep max exercises, score is based on the final 

**ANSWER**
Note that pounds is a unit of force.  Kilograms is a unit of mass. So mass needs to be multiplied by gravity to get force for those who use metric.
...
Open18_1 <- function(kneetowaist, handtoshoulder, weight, bodyweight,  time){
  ...
  rowpower <- 14 * 3085.96 # cals to foot pounds
...
4.  [5] For the affiliate you identified in Homework 1 (one of the top 10 affiliates by number of athletes who competed in both 2018 and 2019 in the state/country that represents where you were born) For each athlete,
