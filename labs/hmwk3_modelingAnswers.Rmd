---
title: 'Homework Transformations and Modeling'
author: "IE 0015"
date: "April 17, 2017"
output:
...
$Value ~ b0 + b1 * time\_period + b2 * seasonality\_factor$
...
```{r}
monthly_sales_of_company_x_jan_6 <- read_excel("C:/Users/lol11/Downloads/monthly-sales-of-company-x-jan-6.xlsx", col_names = c("Month", "Sales"), col_types = c("date", "numeric"), skip=13)
```

  a. [4] You will need to create columnns to represent the trend (total number of months) and the seasonality factor (month of the year). Using the Date field, create three fields: the year, the month of year (numeric), and the month of the study (this will go from 1 to 77).
  ...
  d. [2] Print out the four diagnostic plots: residuals by predicted value, QQ plot of residuals, scale-location of residuals, residuals vs. leveage (Cook's distance)
...
monthly_sales_of_company_x_jan_6 <- monthly_sales_of_company_x_jan_6 %>%
  mutate(monthnum = print(Month, format="%M"))
```
2.  [5] Predictions
...
b.  [2] Plot the confidence interval and prediction intervals. Discuss the results 
