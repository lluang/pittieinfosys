...
```
Note:   a) Table name (e.g. titanic) must be added in order to tell R which table does field belongs to. 
...
```
Note:   a) Table name (e.g. titanic$) should be added in order to tell R which table does the column belongs to. If you didn't add table name, R will create a new vector(e.g. with name pregordr) rather than change the "pregordr" column inside the data frame
