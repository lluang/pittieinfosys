...
Notes: 
  1. To split a string by character position, use `str_sub`. Note that the first three characters of subgroup all all the same so they should be removed.
  2. The total number of characters in a string can be found by `str_length(stringname)`
  3. `str_split` takes a string and splits it using a character to split the string. e.g.
  `splittedstring <- str_split("new_sp_m2534", pattern="_", simplify=TRUE)`
  4. `str_split` with `simplify=TRUE` returns a vector. If you forget `simplify=TRUE`, it returns a list that is harder to work with (it will return a list that is inside of another list, and it accessed by `splittedstring[[1]]`)
  5. Start by looking at a small number of rows (e.g. 5) and get that to work. Then see if it works for all of the rows.
...
3. Check if there are two or three subgroups
4. The first character of the last subgroup is the gender
5. If there are three subgroups, the second subgroup is the diagnosis mode.  If there are two subgroups, the diagnosis mode is "rel". (no diagnosis mode is recorded)
6. The age range comes after the 1st character of the last subgroup.  They can either use all of the numbers to identify the age range, or they can try to pull out the beginning and end.
...
```{r}
tidywho2010$subgroup1 <- str_sub(tidywho2010$subgroup, 4, length(tidywho2010$subgroup))
diagnosismode <- c()
gender <- c()
agerange <- c()
for(newrange in tidywho2010$subgroup1){
  subgroupsplit <- str_split(newrange, pattern = "_", simplify=TRUE)
  diagnosismode <- append(diagnosismode, subgroupsplit[length(subgroupsplit)-1])
  subgrouplast <- subgroupsplit[length(subgroupsplit)]
  gender <- append(gender, str_sub(subgrouplast, 1, 1))
  agerange <- append(agerange, str_sub(subgrouplast, 2, str_length(subgrouplast)))
}
tidywho2010$diagnosismode <- diagnosismode
tidywho2010$gender <- gender
tidywho2010$agerange <- agerange
tidywho2010 %>% head(10)
```
...
Make a table of total diagnosis by country, then combine it with the population table, then get rates.  Note: students may or may not include diagnosismode in their answer.

```{r}
tidywho2010 %>% 
    group_by(country, year, diagnosismode) %>%
      summarize(newcases=sum(cases, na.rm=TRUE)) %>%
        left_join(population) %>%
          mutate(rate=newcases/population*100000) %>% tail(15)
```

Alternative without the diagnosismode column.
