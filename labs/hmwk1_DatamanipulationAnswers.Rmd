...
1. [3] Look the `who` and `population` data sets from the `tidyr` package (the information files are in the `tidyr` package help files).  Make the `who` dataset tidy data (Note: see lab 3) with the following columns:  Country, iso2, year, subgroup, cases.  For `subgroup`, the value should be what are currently column headings. (in lab 3 you used `agerange`)
...
2.  [6] In the subgroup column, there are actually three pieces information: the mode of diagnosis,  gender, and the age range.
