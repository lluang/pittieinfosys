...
```{r}
tidywho2010$subgroup <- substring(tidywho2010$subgroup, 4, length(tidywho2010$subgroup))
diagnosismode <- c()
gender <- c()
agerange <- c()
for(newrange in tidywho2010$subgroup){
  ...
  ifelse(nchar(subgrouplast)==3,
         {
           agerange <- append(agerange, paste(substring(subgrouplast, 2,3), "+", sep="")) 
         },
         ifelse(nchar(subgrouplast)==4,
                {agerange <- append(agerange, paste(substring(subgrouplast, 2,2), "-", substring(subgrouplast, 3,4)) )
                }
                ,
                #nchar(subgrouplast)==5
                {
                  agerange <- append(agerange, paste(substring(subgrouplast, 2,3), "-", substring(subgrouplast, 4,5)) )
