...
Notes: 
  ...
  3. `strsplit` takes a string and splits it using a character to split the string.
  4. `strsplit` returns a list of length
...
```{r}
tidywho$subgroup <- substring(tidywho$subgroup, 4, length(tidywho$subgroup))
subgroupsplit <- strsplit(tidywho$subgroup[1], split= "_")
diagnosismode <- subgroupsplit[[1]][length(subgroupsplit[[1]])-1]  
gender <- substring(subgroupsplit[[1]][length(subgroupsplit[[1]])], 1, 1)

```

3.  [6] You need a table of rates of annual diagnosis by country for the last 5 years (2009-2013).
  ...
  c. Display the first 15 rows (3 countries).
