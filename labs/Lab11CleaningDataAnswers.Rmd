...
One method of filling in missing values is to use a summary value. 

i.  Filter the Titanic data for the entries that are missing the age and inspect a few values (using either `head()` or `tail()` functions.
ii. Plot the age data and give two potential summary values that can be used to fill in the missing value. Discuss why you would use one over the other. Remember to include a title and axis labels.

ANSWER
------

```{r}
ggplot(titanic, aes(Age)) + geom_bar(binwidth=4) +ggtitle("Age distribution of Titanic Passengers") + xlab("Passenger age") + ylab("Number of passengers")
...
#  2.  (4 pts) Hot Deck imputation

i. Create a plot of all pairwise comparisons of variables (hint: search for `pairs` in R help if you do not know how to do this).  What variables provide indications of the age of the passenger? (hint: the plot should give and impression of the distribution, and the distribution should be different for each age).  
ii. Pick one variable and use Hot Deck (random sample) to fill in the age of passenger into the variable `HotDeckAge` in the `titanic` dataframe.
...
```
Note: 
 Do not change the original `Age` Column. Create a new column, for example, the HotDeckAge above. You will need to compare the new one with the origin. Alter the orignal one will also make it necessary to reload data before you start Q3 because the `NA` already been filled in Q2.

#  3.  (3 pts) Mean value and title.

In Lab 7, we used the Name field to extract the title.  Try to use the mean age based on the title and gender to fill in a variable named `titanic$Agetitle`.   
...
Look at the `HotDeckAge` and `Agetitle` variables for the passengers whose age was not available. (hint, use `head()` or `tail()` to sample a few rows)  Which of these two methods of imputing the missing value is more useful?  Why? Can you improve this? (Hint: In early 20th century England "Master" denoted a male child.)
...
When discussing why, we should note that because the hypothesis is *women or children first*, it is only necessary to get an age to identify male children, because if it is a female child, they would already be given priority based on being female.
