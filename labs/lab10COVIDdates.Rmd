...
Download the daily reports data from Courseweb into a directory, and replace the `datadirectory` line with the appropriate directory path.  Use your Windows Explorer or Finder (Mac OSX to assist you if you do not know how).  Note the use of `\\` for separating directory names (escape character to use the `\`).  The alternative is to replace all of the `\` with `/`.
...
a. Using lubridate, convert the date from character to a date format and save the result into a datefield.  (Note:  you will get approximately 560 lines failing to parse and you will get an `NA`, because January data files are in a different date format (DD-MM-YYYY HH:MM)).  Since we are looking at only U.S. data in this lab, you will remove them. Remove them by filtering using `filter(!is.na(datefield))`

**ANSWER**
```{r convert dates}
coviddaily <- coviddaily %>%
  mutate(datetype1 = ymd_hms(`Last Update`))%>%
  filter(!is.na(datetype1))

```

From you answer above, we do not care about the time that the update occured, so use lubridate to change the datefield to only the date, without the time stamp.

**ANSWER**
```{r}
coviddaily <- coviddaily %>%
  mutate(datetype1 = date(datetype1))
```
...
covidus <- coviddaily %>%
  filter(!str_detect(`Province/State`, "Princess"), 
         !str_detect(`Province/State`, "Cruise")) %>%
  filter(`Country/Region`=="US") %>%
  mutate(state = str_extract(`Province/State`, "[A-Z]{2}?"))
```
...
b. Determine the three states with the most confirmed cases. Note that you need to group the data by state to do this. Note: because of NAs, use the option `na.rm=TRUE` for any summary function that needs it.
...
```{r}
covidtop9 <- covidus %>% 
  filter(state %in% top9states$state)
ggplot(covidtop9) +
  geom_line(aes(x=datetype1, y=Infected), color="red") +
  geom_line(aes(x=datetype1, y=Removed), color="blue") + 
  facet_wrap(.~state)+
  scale_x_date() 
```

Note: On occasion, the