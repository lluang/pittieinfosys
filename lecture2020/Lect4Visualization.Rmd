...
-  CoNVO
    -  These are the article discussions we have around every other week (e.g. What is Crossfit?). 
-  Data manipulation (verbs)
    -  Filter, Arrange, Select, Distinct, Mutate, Summarize, Random select
...
-  Load a dataset
-  Diamond sales (included in ggplot library)
```{r}
library(dplyr)
library(magrittr)
library(ggplot2)
...
# Visualize two-dimensions - scatter ploat
...
-The *aesthetic* can be in the main `ggplot()` function or we can specify it in the `geom_()` function.
- We do this because we want to change the statistic represented or because we will plot many things in the same chart.

```{r}
ggplot(dsmall) + geom_point(aes(x=carat, y=price))
```
...
- Can use color or shape of point to add another dimension

```{r}
ggplot(dsmall) + geom_point(aes(x=carat, y=price, color = color))
```
  
`geom_smooth()` with using linetype
=================
...
```
# Adding another dimension using color and linetype
...
Lineplots
===========
- Often used for time series - shows change in values
```{r}
data(economics)
...
Histogram
==========
-  Look at distribution of data over a single axis
```{r}
ggplot(dsmall) + geom_histogram(aes(carat))
```


Boxplots
==============
- Five number summary of a numerical variable
- Minimum, first quartile, median, third quartile, maximum
- *whiskers* are 1.5 times interquartile range, if this does not reach the maximum value.
```{r}
ggplot(dsmall) +
...
- Distribution over a categorical variable (compare to histogram over a numerical variable)

```{r}
ggplot(dsmall) + geom_bar(aes(x=color))
```

Facets
===========
- You can use multiple aesthetics to show dimensions, or split the data using *facets*
- E.g. scatterplot, split by cut
- `facet_grid` either horizontal or vertical stacked
- `facet_wrap` creates a 2d grid

```{r}
ggplot(dsmall) + geom_point(aes(x=carat, y=price)) + facet_grid(. ~ cut)
```
...
```{r}
ggplot(dsmall) + geom_point(aes(x=carat, y=price)) + facet_wrap(. ~ cut)
```

Overlaying representations
=================

- You can instead overlay multiple representations of the same data to provide more information.
