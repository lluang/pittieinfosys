...
- It will help to have R Studio open and follow along in this lecture.
- Download ACS housing and personal data as well as the data dictionary
...
read_csv("hogwarts.csv") # from readr
read_xlsx("hogwarts.xlsx", sheet="Sheet1") #from readxl
read.fwf("hogwarts.dat")
```

Read housing and personal record
===================

```{r readhousing}
library(readr)
acshousing <- read_csv("../data/ss12hpaPittsburgh01701.csv")
head(acshousing[,1:10])
```
```{r readpersonal}
acspersonal <- read_csv("../data/ss12ppaPittsburgh01701.csv")
head(acspersonal[,1:10])
...
-  Other packages include *readr* for text (csv) files, *readxl* (for Excel spreadsheets), *ggplot2* (for charts), and *dplyr* and *reshape* (for data manipulation)
...
- Excel spreadsheets can be read using the package *readxl* (included in `tidyverse`)
...
colnames(tenure) <- c("TEN", "description")
```
...
```{r gettenure}
acshousing$TEN <- as.factor(acshousing$TEN)
acshousing %>% 
  left_join(tenure) %>% 
    select(SERIALNO, PUMA, ST, TEN) %>%
      head()
```
...
- Remove duplicates from a dataset.  E.g. get distinct houses

```{r}
hogwartstudents %>% select(house) %>% distinct()
```
...
```{r}
acshousing %>%
  mutate(incomeper=ADJINC/NP) %>%
  select(SERIALNO, ADJINC, NP, incomeper) %>%
  head()
```
...
- Sampling can specify number or probability
...
```{r studentsrandomfrac}
hogwartstudents %>%
  sample_frac(.3)
```

two-table verbs (JOIN)
======

- We can replace all of the `JOIN` operations from SQL in R as well.
- One way is to use logical statements in the brackets (as in above)
- Another way is to use the `join` and `filter` statements from the `dplyr` package (part of `tidyverse`)

```{r}
acshousing %>%
  left_join(acspersonal, by="SERIALNO") %>% 
  filter(FES %in% c("5", "6", "7", "8")) %>%
  select(SERIALNO, SPORDER, FES, AGEP, HINCP) %>%
  head()
```
...
# dsmall = diamonds[sample(nrow(diamonds),200),] also works
```
...
- `alpha` adjusts the transparency, so it is more apparent that multiple data points overlap.
```{r}
