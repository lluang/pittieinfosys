---
title: "Probability and Monte Carlo simulation"
...
- Load Lecture 10 Probability from Courseweb
- Sign into TopHat 377821 for class discussion

Dr. Seuss
===

![](figures/seussismzoom.jpg)

Case final Report
=====

1. Format is in Course Documents. Note that the case and final project formats are the same specifications.
1. For the Case, the question is if RegionEX delays are out of ordinary. Because all you are doing is data analysis (no modeling that you are learning in other classes), you need to do comparisons. The only benchmark comparison data you have is MDA. So
Through data analysis (EDA) determine what kinds of flights have high rates of RegionEx delays.
Compare RegionEx flights to comparable flights for MDA for those types of flights.  
2. Remember comparisons should include a measure of centrality (e.g. mean, median) and a measure of variation (e.g. standard deviation, IQR)
    i. Differences in measure of variation usually imply that there is a process standardization problem
    ii. Differences in measure of centrality usually imply a process improvement problem
    iii. If the comparisons are not fair,  (e.g. one side does not see the same conditions as the other or the rules are different) state that as well.
3. From that comparison, you may be able to make conclusions on where RegionEx can improve (i.e. it is fair to say that RegionEx should be able to match a comparable benchmark)

4 V's of Big Data
=================

-  Volume - How many observations?  What constitutes an observation
-  Variety - What information is available on each observation?  Are they very similar, or do they describe a range of different aspects of the observation
-  Velocity - Is data added over time? Does the underlying environment that produced the data change?  Does data expire?  Is the Need time sensitive (you can only use recent data)
-  Veracity - Is the data accurate?  Are there missing values or data errors?  Is data collection complete or are there missing observations?  Are data fields defined well enough so that entries mean what they seem to mean.
...
- This week: Probability and Monte Carlo simulation
- Next week: Boot strap and Time step simulation
- Last week: R and Industrial engineering curriculum

Simulations are forms of models
====================================
...
-  Models are intended to be abstractions of reality.
-  A model is developed to answer a specific question(s)
    - Validation:  Does the form of the model useful for answering the question at hand?
...
- `cdfplot` shows cumulative distribution function
...
2.    This is on the basis of the shape of the family (i.e. be familiar with the pictures found in most probability books.  Look at a histogram or density plot.
3.    Next, use prior knowledge about the source of the random variable.  i.e. the process that generates the random event.
      ...
      - If independent customers are arriving and you want the number within a given time interval, Poisson.
      ...
      - Consider if the distribution should be bounded (is there a maximum or minimum).
...
-  We can use samples of random numbers for simulating settings.
    -  Called *Monte Carlo Simulation*, after the city on the Mediterranean known for gambling.
