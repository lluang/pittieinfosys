---
title: 'Lecture 7: Advanced programming concepts: Functions'
...
1. Pipes are long (e.g. longer than 10 steps). Difficult to debug if something goes wrong. If so, break the pipe into segments, with each intermediate statements generating objects with meaningful names.
...
3. The process is not a linear construct. Although, you could write a function that manages the branching to be called with the `mutate` function to make this work.
...
- Note: the computer does not care what you call your function or the names of the variables, but you and your teammates will care when you try to understand what you did.
- So will you remember what a name means a month from now. Or six months from now. Or next year.
...
- Generally, function names should be verb phrases, variable names should be noun phrases.
    - Exception, when a lot of your functions seem to start with the same verb you can be creative.
