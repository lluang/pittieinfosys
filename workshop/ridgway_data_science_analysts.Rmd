...
-  What is the alternative to the analyst?
...
ggplot(data = all_alaska_flights, mapping = aes(x = dep_delay, y = arr_delay)) + 
  geom_point() + ggtitle("Flights arrival delay vs departure delay")
```
...
ggplot(data = early_january_weather, mapping = aes(x = time_hour, y = temp)) +
  geom_line() +ggtitle("Weather in January")
```
...
ggplot(data = weather, mapping = aes(x = factor(month), y = temp)) +
  geom_boxplot() +ggtitle("Temperature by month")
```
...
- You are preparing data for use in analysis, not to make a table for presentation
...
6.  Mutate (calculations)
7.  Summarize (sum, average, etc.)
8.  Random sample (for simulation)
