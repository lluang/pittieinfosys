...
2. *p*dist - CDF. Given the list of quantiles (values of x) and parameters of the distribution, return the corresponding cumulative probabilities.
3. *q*dist - Quantile. Given the list of probabilities and parameters of the distribution, return the corresponding quantiles (values of x)
4. *r*dist - Random. Given the parameters of the distribution and a required number, generate random values from that distribution.


# Triangular distribution

- There are various packages for distributions that are used in specific situations that are not included in base R.
...
# Triangular distribution plot

- *d* function

```{r}
xval <- seq(0.1, 10, by = 0.1)
yval <- dtriangle(xval, 0, 10, 4)
```
```{r, echo=FALSE}
triangleexample <- data.frame(xval, yval)
ggplot(triangleexample) + geom_point(aes(xval, yval)) + theme_bw() + ggtitle("density  plot generated graph")
```
...
xval <- ptriangle(yval, 0, 10, 4)
```
```{r, echo=FALSE}
triangleexample <- data.frame(xval, yval)
ggplot(triangleexample) + geom_point(aes(xval, yval)) + theme_bw() + ggtitle("Probability plot generated graph")
```


# Triangular quantile plot

- *q* function

```{r}
xval <- qtriangle(yval, 0, 10, 4)
```
```{r, echo=FALSE}
triangleexample <- data.frame(xval, yval)
ggplot(triangleexample) + geom_point(aes(xval, yval)) + theme_bw() + ggtitle("Probability plot generated graph")
...
# Triangular random number plot

- *r* function generates random numbers from that distribution

```{r}
xval <- rtriangle(100000, 0, 10, 4)
```
```{r, echo=FALSE}
triangledensity <- data.frame(xval)
ggplot(triangledensity) + geom_histogram(aes(xval)) + theme_bw() + ggtitle("Triangle distribution")
```
