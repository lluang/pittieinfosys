...
- If you need to take dates and group them (e.g. into months) you can use the `cut()` function with dates. (actually, uses `cut.Date` when `cut` recognizes it is being used with `Date` formatted data)


```{r}
HealthyHarborWaterWheelTotals2017_1 <- read_excel("../data/HealthyHarborWaterWheelTotals2017-1.xlsx",
skip = 1)
waterwheeldata <- HealthyHarborWaterWheelTotals2017_1 %>% 
  filter(!is.na(Dumpster))
waterwheeldata$monthly <- as.Date(cut.Date(as.Date(waterwheeldata$Date), breaks="month"))
```
```{r}
waterwheeldata <- waterwheeldata %>%
  group_by(monthly, Date) %>%
  summarize(Dailyweight = sum(`Weight (tons)`), Dailyvolume = sum(`Volume (cubic yards)`))
```
...
====================
```{r}
ggplot(FemPreg[(FemPreg$BIRTHWGT_LB1<99 & !is.na(FemPreg$BIRTHWGT_LB1)),], aes(as.factor(pregordr), BIRTHWGT_LB1)) +
  geom_boxplot() + xlab("Pregnancy order") + ylab("Birthweight (lbs)") + ggtitle("Birthweight by pregnancy order as factor")
```
```{r}
ggplot(FemPreg[(FemPreg$BIRTHWGT_LB1<99 & !is.na(FemPreg$BIRTHWGT_LB1)),], aes(as.integer(pregordr), BIRTHWGT_LB1, group=pregordr)) +
  geom_boxplot() + xlab("Pregnancy order") + ylab("Birthweight (lbs)") + ggtitle("Birthweight by pregnancy order as integer")
```
Summarizing factor variables
===========

```{r}
summary(as.factor(FemPreg$pregordr))
```
```{r}
summary(as.integer(FemPreg$pregordr))
```
...
- the `stringr` package makes working with strings easier
- Includes functions for:
  i. Character manipulation
  ii. Whitespace tools
  iii. Locale specific
  iv. Pattern matching
...
Things you may want to do:
  ...
  2. Get or set individual characters
  3. Modify substrings
...
str_length("abc")
x <- c("abcde", "ghifjk")
# the 3rd character of each item in x
str_sub(x,  3, 3)
# the 2nd to 2nd to last character of each item in x
str_sub(x, 2, -2)
```
...
You can add, remove or modify whitespace

- `str_pad()` pads a string by adding whitespace to left, right or both sides to make the string a specific length
```{r}
str_pad(x, 10)
str_pad(x, 10, "right")
str_pad(x, 10, "both")
...
- You can also use other characters besides a space
...
- Some parts of the world/publications use different capitalization rules for titles and sorting
...
str_to_title(horses)
str_to_lower(horses)
str_to_lower(horses, locale="tr") # Turkish i without a dot
```
...
```{r}
someletters <- c("y", "i", "k")
str_sort(someletters)
str_sort(someletters, locale="lt") # Lithuanian
```

Pattern Matching
==============

- Uses regular expressions (like `regexpr` and `gregexpr`)
