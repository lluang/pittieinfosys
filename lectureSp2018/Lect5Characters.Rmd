---
title: "Lecture 5: Character Manipulation and Regular Expressions"
author: "IE 0015"
date: "January 2017"
output:
...
```
- Add the *sep* arguement to separate them (other than tabs)
- Add the *collapse* arguement to combine them into a single string.
...
-  Look at *help(str_sub)* to see arguements for *str_sub* (start, end)
...
str_view_all(numbers, regex("[\\W]?[0-9]{3}[\\W]?[0-9]{3}[\\W]?[0-9]{4}"))
```
...
```{r}
data = titanic$name[5]
startpoint = regexpr(", [A-Z][a-z]*\\.", data)
attributes(startpoint)
attr(startpoint, 'match.length')
title = str_sub(data, startpoint+2, startpoint + attr(startpoint, 'match.length')-1)
title
```
